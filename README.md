...
It was written by Andy Woodruff, Ryan Mullins and Cristen Jones for [Maptime Boston](https://twitter.com/maptimeboston) and, although it's been updated a few times, you'll need to update a few things as you go.

## Remember all that boring stuff in class about maintenance?

As we’ve discussed in class, an important thing to think about when writing code is sustainability. Over time, code gets ‘stale’ and, if not updated, can break. While the Maptime Boston code still works, a few of the libraries have been deprecated by Github and these examples won’t work in Github pages. That’s where this repo comes in: outlined below are the changes you need to make as you read through the tutorial (don’t worry, there are only a few). This repo also contains updated code for all maps in the tutorial. [You see them rendered on this page](https://shadrock.github.io/first-leaflet-map/).
...
## Replacement No. 2
In the section labeled, “Easy! Now let's map our own data!” you’ll download a file for jQuery and reference in your code like this.
...
You’ll change this by downloading an updated .js file, which you can find in the Leaflet-heat Github repo. https://github.com/Leaflet/Leaflet.heat Then change your code to reference this. The could should be changed to
